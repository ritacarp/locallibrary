...
exports.BACKUP_genre_list = async function (req, res, next) {
  ...
  try {
    ...
    for (let i = 0; i < genres.length; i++) {
      ...
      console.log("\n\n");
      console.log(
        "count of books for genre " + genres[i].name + " = " + bookCount
      );
      console.log("\n\n");
  ...
  res.render("BACKUP_genre_list", {
    title: "BACKUP Genre List",
...
exports.genre_update_get = function (req, res, next) {
  ...
  Genre.findById(req.params.id).exec(function (err, genre) {
    if (err) {
      console.log(
        "Edit Genre: GET - count not find genre by id " + req.params.id + "\n"
      );
      return next(err);
    }
    //Successful, so render
  ...
  check("name").custom((value) => {
    if (value.match(/^[A-Za-z ]+$/)) {
      ...
      return Promise.reject(
        "Genre name must not have non-alphanumeric characters."
      );
    }
