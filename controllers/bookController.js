    ...
    Book.find(vSearchQuery, { title: 1, author: 1 })
      ...
      .exec(function (err, list_books) {
        ...
        Book.countDocuments(vSearchQuery, function (err, bookCount) {
          ...
          res.render("BACKUP_book_list", {
            title: "BACKUP Book List",
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      if (req.query.authorID) {
        authorIDs = req.query.authorID;
      }

      let genreIDs = "";
      if (req.query.genreID) {
        genreIDs = req.query.genreID;
      }
...
function findBook(
  ...
  Book.findOne(titleAndAuthorQuery).exec(function (err, found_titleAndAuthor) {
    if (err) {
      return next(err);
    }
    if (found_titleAndAuthor) {
      ...
      res.render("book_form", {
        ...
        duplicate: duplicate,
        duplicateBookURL: duplicateBookURL,
      });
    } else {
      Book.findOne(isbnQuery).exec(function (err, found_isbn) {
        if (err) {
          return next(err);
        }
        if (found_isbn) {
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      Book.findById(req.params.id).exec(function (err, book) {
        if (err) {
          console.log(
            "Edit Book: GET - could not find author by id " +
              req.params.id +
              "\n"
          );
          return next(err);
